"""
strategy v2 - same pipeline, selection dial: add the rank buffer
================================================================
One change from v1, in CONFIG only:

  * keep_rank: None -> 20, and n_stocks: 8 -> 7

The selection rule goes from "fresh top-N every review" to "hold a
stock while it still ranks inside the top 20; sell only when it
drops out; fill empty slots with the best newcomers". Effect:
quarterly turnover roughly halves and multi-quarter trends stay in
the book -- exactly the churn problem in the v1 notes. 20 is a round
number close to 3x the book size, not a tuned constant -- worth
re-checking under stress later.

Result (2021-25): Rs 12.8 Cr PNL (v1: 8.6), MDD improves to -27%.
H1-2026 check: +22.3%.

Working note for tomorrow: the -15% stop still costs about 1 Cr
versus no stop at all (12.8 vs 13.9 in a quick A/B), and almost
every exit in the log is an ordinary dip that recovered -- the stop
level is the last dial to set.
"""
...
if __name__ == '__main__':
    px = load_prices()
    print('strategy v2 | 2021-25 backtest:')
    run_backtest(px, '2021-01-01', '2025-12-31')
    print('strategy v2 | H1-2026 check:')
